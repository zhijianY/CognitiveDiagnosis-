...
class Net(nn.Module):
    ...
    def __init__(self, knowledge_n, exer_n, student_n):
        ...
        ##使用view 对应KANLinear网络输入
        # self.student_emb = torch.view(-1,student_emb)
        self.k_difficulty = nn.Embedding(self.exer_n, self.knowledge_dim)
        self.e_difficulty = nn.Embedding(self.exer_n, 1)
        self.degree = 5
        # self.prednet_full1 = PosLinear(self.prednet_input_len, self.prednet_len1)
        self.prednet_full1 = ChebyKANLayer(self.prednet_input_len, self.prednet_len1,self.degree)
        self.drop_1 = nn.Dropout(p=0.5)
        # self.prednet_full2 = PosLinear(self.prednet_len1, self.prednet_len2)
        self.prednet_full2 = ChebyKANLayer(self.prednet_len1, self.prednet_len2,self.degree)        
        self.drop_2 = nn.Dropout(p=0.5)
        # self.prednet_full3 = PosLinear(self.prednet_len2, 1)
        self.prednet_full3 = ChebyKANLayer(self.prednet_len2, 1,self.degree)
...
class cheKCDM(CDM):
    '''cheKCDM Network Cognitive Diagnosis Model'''

    def __init__(self, knowledge_n, exer_n, student_n):
        super(cheKCDM, self).__init__()
